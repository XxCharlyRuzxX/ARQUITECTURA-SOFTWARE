...
public class CsvReader {
    ...
    public ArrayList<MessageParticipants> getCsvMessageParticipants() {
        try (BufferedReader reader = new BufferedReader(new FileReader(this.filePath))){
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if(lineNumber == 1){
                    try {
                        validLine = csvLineFormatValidator.checkCsvIdentifiers(data);
                        lineNumber++;
                        continue;
    ...
    /* Convierte la linea en un objeto tipo MessagePAtricipants, asignando los nombres dependiendo de las keys de la primera fila */
